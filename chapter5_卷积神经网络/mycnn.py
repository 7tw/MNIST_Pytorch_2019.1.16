...
for epoch in range(num_epoches):
    ...
    for i, data in enumerate(train_loader, 1):  # 批处理
        ...
        #原版 running_loss += loss.data[0] * label.size(0)  # total loss , 由于loss 是batch 取均值的，需要把batch size 乘回去
        ...
        #原版 running_acc += num_correct.data[0]  # 正确结果的总数
...
for data in test_loader:  # test set 批处理
    ...
    #原版 img = Variable(img, volatile=True)  # volatile 确定你是否不调用.backward(), 测试中不需要
    #原版 label = Variable(label, volatile=True)
    #先加一行with torch.no_grad():，再把, volatile=True这块删掉
    with torch.no_grad():
    ...
    #原版 eval_loss += loss.data[0] * label.size(0)  # total loss
    eval_loss += loss.item() * label.size(0)  # total loss
    _, pred = torch.max(out, 1)  # 预测结果
    num_correct = (pred == label).sum()  # 正确结果
    #原版 eval_acc += num_correct.data[0]  # 正确结果总数
